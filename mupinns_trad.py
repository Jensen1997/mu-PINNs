...
class NeuralNetwork(nn.Module):
    def __init__(self, Setting, INNER, INLET, OUTLET, WALL, U0):
        ...
        self.in_dim = 3
        self.out_dim = 3 # traditional scheme
        self.tanh = nn.Tanh()
        # uv net
    ...
    def net_f(self, x, y, mu):
        ...
        u_yy = self.gradients(u_y, y)

        v_x = self.gradients(v, x)
        v_y = self.gradients(v, y)
        v_xx = self.gradients(v_x, x)
        v_yy = self.gradients(v_y, y)

        p_x = self.gradients(p, x)
        p_y = self.gradients(p, y)
    ...
    def loss_calculate(self, x, y, mu):
        ...
        f_pred_c, f_pred_u, f_pred_v = self.net_f(x, y, mu)
